...
export default function HomePage({ days, speakers, settings, siteName }: HomePageProps) {
  ...
  const dayTabs = days.map((d) => {
    ...
    return {
      label: `${dt.getMonth() + 1}/${dt.getDate()} (${weekdayNames[dt.getDay()]})`,
      date: `${dt.getMonth() + 1}.${dt.getDate()}`,
      weekday: `星期${weekdayNames[dt.getDay()]}`,
      theme: d.titleZh,
    };
  });
  ...
  const daySessions: SessionItem[][] = days.map((day) =>
    day.sessions
      ...
      .map((s: any) => ({
        ...
        duration: s.duration ? `${parseInt(s.startTime) < 12 ? "上午" : "下午"} · ${s.duration} 分鐘` : undefined,
        type: getTypeLabel(s.type, s.titleZh),
        badge: getBadge(s.type),
        title: s.titleEn || s.titleZh || undefined,
        speakers: s.sessionSpeakers?.filter((ss: any) => ss.role !== "discussant").map((ss: any) => ({
          name: ss.speaker.nameEn || ss.speaker.name,
          role: ss.role === "moderator" ? "Moderator" : undefined,
        })) || undefined,
        papers: s.papers?.length > 0 ? s.papers.map((p: any) => ({
          title: p.titleEn || p.titleZh,
          author: p.speaker?.nameEn || p.speaker?.name || "",
          affiliation: "",
        })) : undefined,
        discussants: s.sessionSpeakers?.filter((ss: any) => ss.role === "discussant").length > 0
          ? s.sessionSpeakers.filter((ss: any) => ss.role === "discussant").map((ss: any) => ss.speaker.nameEn || ss.speaker.name)
          : undefined,
  ...
  const descriptionData = {
    ...
    body: settings.description_body || "「全球共善學思會」匯聚國際佛學研究者、宗教實踐者與人文學者，以學術發表、圓桌論壇與沉浸式藝術體驗，深入探討應用佛教、菩薩道精神與佛教藝術的當代轉譯。本次學思會由慈濟基金會與哈佛大學 CAMLab 共同主辦，期盼在學術對話中，為佛教的未來開展新的視野與可能。",
    highlights: [
  ...
  const speakerList = speakers.map((s: any) => ({
    name: [s.name, s.nameEn].filter(Boolean).join(" "),
    ...
    tags: s.sessionSpeakers?.map((ss: any) => {
      ...
      const role = ss.role === "moderator" ? "Moderator" : ss.role === "discussant" ? "Commentator" : ss.session.titleZh;
  ...
  const [selectedSpeaker, setSelectedSpeaker] = useState<(typeof speakerList)[number] | null>(null);
